<?php
    ...
    mysqli_select_db($con, "cadastrodealunos");
...
<?php
    ...
    if(empty($_POST['txt_nome']) || strstr ($_POST['txt_nome'], ' ') == false) {
        echo "Favor digitar seu nome completo.<br><br>"; 
        $erro=true;
    } else {
        echo "Nome: ", $_POST['txt_nome'], "<br><br>";
    }

    if(empty($endereco) || strstr ($endereco, ' ') == false) {
        echo "Favor digitar seu endereço completo.<br><br>"; 
        $erro=true;
    } else {
    ...
    if (strlen($comentario) < 1) {
        echo "Favor digitar um comentário suas experiências com Inglês!<br><br>";
        $erro=true;
    } else {
